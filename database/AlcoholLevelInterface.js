...
m.deleteAllAlcoholLevels = function(error, cb) {
  AlcoholLevel.find({}, function(err, alcoholLevels) {
    if (err) {
      error(err);
    } else {
...
m.getAllAlcoholLevelsGreaterThanTime = function(time, error, cb) {
  var query = AlcoholLevel.find().sort({
    time: -1
  });
  query.exec(function(err, alcoholLevelEntries) {
    if (err) {
      error(err);
    } else {
      var ex = [];
      for (var i in alcoholLevelEntries) {
